...
public class RecordFragment extends BaseFragment {
	...
	private void initView(View v, ViewGroup vg) {
		...
		mInfo.clear();
		mDeals.clear();
		mDeals = DBUtil.getOperation(getActivity()).findAll(DealInfo.class);
		mInfo.addAll(EnvUtil.parseOrderInfo(mDeals, mDaGetData));
			...
			public void onItemClick(final AdapterView<?> parent, View view,
				...
				dialog.setRevokeClick(new OnClickListener() {
					...
					public void onClick(View v) {
						// TODO Auto-generated method stub
						dialog.dismiss();
						if(((OrderInfo)parent.getItemAtPosition(position)).getRemark().equals("已退款")){
							ViewUtil.showError("该笔订单已退款，不可以重复退款！", getActivity());
							return;
						}
						...
						UpayTask.getInstance(getActivity()).revoke(
								mInfo.get(position - 1), new UpayCallBack() {
									...
									public void onExecuteResult(UpayResult arg0) {
										...
										if (arg0.getState() == 2) {
											mInfo.get(position - 1).setRemark(
													"已退款");
											mDeals.get(position - 1).setState(
													"已退款");
